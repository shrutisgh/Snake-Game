...
class SnakeGame:
    def __init__(self) :
        ...
        self.AI_X=50
        self.SNAKE_LENGTH=0
        self.SNAKE_COORDINATE=[]
        self.SCORE=0
        self.AI_LEVEL=0
        self.AI_Y=50
        self.AI_SNAKE_COORDINATE=[]
    ...
    def Snake_Die(self):
        # Checking if coordinates self.SNAKE_HEAD_COORDINATE(carrying current coOrdinate of head) are in the self.SNAKE_COORDINATE[:-1](list carrying coordinates of each circular unit of snake except the last(head) one)
            ...
            if self.SNAKE_HEAD_COORDINATE in self.SNAKE_COORDINATE[:-1]:
                if self.SNAKE_HEAD_COORDINATE == self.SNAKE_COORDINATE[-1]:
                    self.X+=10
                    self.Y+=10
                 
                    
                else:
                    print(self.SNAKE_COORDINATE[:-1].index(self.SNAKE_HEAD_COORDINATE))
                    # Calling self.Notifier funtion to tell the player that he has collided 
    ...
    def Snake_Length(self):
        self.SNAKE_HEAD_COORDINATE=[]
        self.SNAKE_HEAD_COORDINATE.append(self.X)
        self.SNAKE_HEAD_COORDINATE.append(self.Y)
        self.SNAKE_COORDINATE.append(self.SNAKE_HEAD_COORDINATE)
        ...
        if len(self.SNAKE_COORDINATE)>self.SNAKE_LENGTH:
            del self.SNAKE_COORDINATE[0]
        # Calling self. Snake_Die() which is carrying the criteria to declare when the snake collapsed
        self.Snake_Die()
    # Function managing coordinates of PLAYER's snake and its length     
    def AI_Snake_Length(self):
        self.AI_SNAKE_HEAD_COORDINATE=[]
        self.AI_SNAKE_HEAD_COORDINATE.append(self.AI_X)
        self.AI_SNAKE_HEAD_COORDINATE.append(self.AI_Y)
        self.AI_SNAKE_COORDINATE.append(self.AI_SNAKE_HEAD_COORDINATE)
        # Condition to keep a check on length of lists with respect to length of snake, if number of elements in list is more than self.SNAKE_LENGTH than deleting the first element
        if len(self.AI_SNAKE_COORDINATE)>self.AI_SNAKE_LENGTH:
            del self.AI_SNAKE_COORDINATE[0]
    ...
    def AI_Snake(self):
        ...
        for x, y in self.AI_SNAKE_COORDINATE:
    ...
    def Player(self):
        for x, y in self.SNAKE_COORDINATE:
